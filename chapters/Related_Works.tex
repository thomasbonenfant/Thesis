...
% In this chapter we review an optimization based approach to the air hockey trajectory planning
% and the main challenges in the application of reinforcement learning in the
% field of robotics, describing a state of the art algorithm for safe exploration.

In this chapter, we review an optimization-based approach to air hockey trajectory planning and examine the main challenges in applying reinforcement learning within robotics. We also describe a state-of-the-art algorithm for safe exploration.
...
In robotics, the desired behavior of a robot is often defined through planned trajectories with specific structures, such as cubic splines. 
These trajectories are then optimized based on various criteria, including path smoothness, proximity to obstacles, travel time, path length, 
and adherence to the robot's constraints.
